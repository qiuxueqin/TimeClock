...
5. 全部目标条目完成时，页面展示今日已完成状态。若后端仍要求显式确认今日打卡，底部按钮才可用；若后端自动创建打卡记录，前端仅展示结果，不再二次写入。
...
5. 上传成功跳转或刷新至批次详情；解析中以 5 秒间隔轮询 `GET /imports/{batchId}`，在 `PARSED`、`FAILED`、`CANCELLED` 终态停止轮询，离开页面即取消。
