...
export function ItemPage() {
  ...
  const complete = useMutation({ mutationFn: ({ id, text }: { id: string; text: string }) => submissionApi.complete(id, text), onSuccess: async (res) => { await queryClient.invalidateQueries({ queryKey: ['items', taskId] }); await queryClient.invalidateQueries({ queryKey: ['today-items', taskId] }); message.success(res.checkinStatus === 'completed' ? '已完成，今日打卡达成' : '已完成'); }, onError: (e: Error) => message.error(e.message) });
  const reopen = useMutation({ mutationFn: (id: string) => submissionApi.reopen(id), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ['items', taskId] }); await queryClient.invalidateQueries({ queryKey: ['today-items', taskId] }); message.success('已撤销完成'); }, onError: (e: Error) => message.error(e.message) });
